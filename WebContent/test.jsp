	...
	<script type="text/javascript">
		var file = {
			...
			callback : function(dataURL) {
				...
				var url="upPic.do"
				//这里必须将ajax的异步改为同步才可以把返回并保存在隐藏域中的图片地址取出同时加在地址栏中作为参数一并传入下一个页面，这样做的目的是因为返回的图片地址不是一个json数组，而是单个的json字符串，所以只能将返回的图片地址json字符串拼接在一起作为参数传到下一个页面     
				$.post(url, dataURL, function(res) {
					//将base64图片流的图片通过后台转换成普通的图片路径并上传至服务器     
					var imgUrl = $("#hiddenImgUrl").val();
					if (res.success) {
						$(".loading").hide();
						if (imgUrl != "") {
							$("#hiddenImgUrl").val(imgUrl + "|" + res.imgUrl);
							//中间加一个 | 是为了到下一个页面便于将传过去的图片地址参数转换为数组     
						} else {
