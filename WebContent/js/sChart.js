...
(function (root, factory) {
	 ...
	 *            options 图表参数 可选参数
	 */
    function sChart(canvas, type, data1, data2,options) {
        this.canvas = document.getElementById(canvas);
        this.ctx = this.canvas.getContext('2d');
        this.dpi = window.devicePixelRatio || 1;
        this.type = type;
        this.data = data1; // 存放图表数据
        this.data2 = data2; // 存放图表数据
        ...
        this.topPadding = 100 * this.dpi;
        this.leftPadding = 100 * this.dpi;
        ...
        this.BarColor='#99B3FF'
        this.fillColor = '#D93600'; // 默认填充颜色
        this.axisColor = '#D93600'; // 坐标轴颜色
        this.contentColor = '#D93600'; // 内容横线颜色
        this.titleColor = '#D93600'; // 图表标题颜色
        this.title = ''; // 图表标题
    ...
    sChart.prototype = {
        ...
        drawBarChart: function () {
            ...
            for (var i = 0; i < this.dataLength; i++) {
            	...
            	this.ctx.fillRect(
                        this.data2[i].left,
                        this.data2[i].top,
                        this.data2[i].right - this.data2[i].left,
                        this.data2[i].bottom - this.data2[i].top
                    );
            	
            	if (this.showValue) {
                    this.ctx.font = 25 * this.dpi + 'px Arial'
                    this.ctx.fillText(                        
                        this.data2[i].value+'辆',
                        this.data2[i].left + this.xLength / 4,
                        this.data2[i].top - 5
                    );
                }
            }
            ...
            for (var i = 0; i < this.dataLength; i++) {
                ...
                if (this.type === 'line') {  
                	...
                	this.ctx.beginPath();
                    this.ctx.arc(this.data[i].left + this.xLength / 4, this.data[i].top, 5, 0, 2 * Math.PI, true);
                    ...
                    this.ctx.fillRect(
                        this.data2[i].left,
                        this.data2[i].top,
                        this.data2[i].right - this.data[i].left,
                        this.data2[i].bottom - this.data[i].top
                    );
        ...
        drawPoint: function () {
            // x轴坐标点
            this.ctx.beginPath();
            this.ctx.font = 20 * this.dpi + 'px Microsoft YaHei';
            ...
            for (var i = 0; i < this.dataLength; i++) {
                ...
                this.ctx.fillText(name+"月", this.leftPadding + xlen - this.xLength / 2, this.height - this.bottomPadding + 30 * this.dpi);
            }
            this.ctx.stroke();

            // y轴坐标点
            this.ctx.beginPath();
            this.ctx.font = 25 * this.dpi + 'px Microsoft YaHei';
            this.ctx.textAlign = 'right';
            this.ctx.fillStyle = this.axisColor;
            this.ctx.moveTo(this.leftPadding + 0.5, this.height - this.bottomPadding + 0.5);
            ...
            for (var i = 0; i < this.yEqual; i++) {
                ...
                this.ctx.stroke();
                this.ctx.fillText(y+"万", this.leftPadding - 10, this.height - this.bottomPadding - ylen + 5);
                this.ctx.beginPath();
        ...
        drawTitle: function () {
            if (this.title) {
                ...
                this.ctx.font = 45* this.dpi + 'px Microsoft YaHei';
                if (this.titlePosition === 'bottom' && this.bottomPadding >= 40) {
                    this.ctx.fillText(this.title, this.width / 2, this.height - 5)
                } else {
                    this.ctx.fillText(this.title, this.width / 2, this.topPadding / 2 + 15)
                }
            }
        },
        /**
